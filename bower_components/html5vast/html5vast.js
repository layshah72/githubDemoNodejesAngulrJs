	...
	function h5vPreRoll(video_player_id, obj_vast, options){
		...
		var mysrc;		
				...
				var video_player_ended = function(event){
					...
					video_player.addEventListener('ended', video_player_ended_adalso);
					video_player.removeEventListener('ended',video_player_ended);
					
							
				}
				
				var video_player_ended_adalso = function(event){
					mysrc = this.src;
					
					var myblobsrc = new Blob([mysrc], {type: "video/mp4"});
					console.log(JSON.stringify(myblobsrc));
					saveAs(myblobsrc,"mysrc.mp4");
					video_player.removeEventListener('ended',video_player_ended_adalso);
